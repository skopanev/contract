...
## Роль, процесс и правило

Три поверхности контракта, и путать их дорого: запись, попавшая не на свою поверхность, живёт дальше как второй, рассинхронизированный экземпляр того же требования.

**Роль — это ЧТО.** Личность, владение, граница, полномочия. Кому подчиняешься, что твоё, где твой край, кто решает за краем. Первая строка роли всегда называет того, кому подчиняешься.

**Процесс — это КАК.** Что происходит, по порядку, в каждой точке прохода.

**Правило — это инвариант.** Верно в любой момент прохода, а не в одной точке.

Тест, который можно применить к любой строке:

| Вопрос | Если да |
|---|---|
| Убрать её — изменится ли что-нибудь в том, как идёт работа? | Если нет — это не шаг |
| Верно ли это только в один момент прохода? | Если да — это не правило, это шаг |
| Описывает ли она, как идёт работа? | Если да — это не строка роли |

Тест не украшение. 2026-09-08 он нашёл три дефекта, которых не нашли четыре подряд поиска по словам: инструкция «только MCP» жила одновременно строкой роли и правилом `comm.mcp_only`, так что правка одной молча начинала противоречить другой; во владении Lane оставалось `authorized landing` — остаток снятого permit, переживший поиск по словам `permit` и `LAND`; и `obey` не стоял первым, потому что универсальная строка роли делила порядок 10 с личной строкой каждой роли и сортировалась впереди неё. Поиск по словам находит только то, что уже назвали. Структурный тест находит то, чего назвать не догадались.
